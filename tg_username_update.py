...
dizzy = emojize(":see_no_evil:", use_aliases=True)
cake = emojize(":lollipop:", use_aliases=True)
    ...
    while True:
        try:
            ...
            if seco=='00' or seco=='30':
                ...
                if for_fun < 0.10:
                    last_name = '%s時%s分 %s' % (hour, minu, hsym)
                elif for_fun < 0.30:
                    last_name = '%s:%s %s %s %s' % (hour, minu, p, abbwn, hsym)
                elif for_fun < 0.60:
                    last_name = '%s:%s %s EST %s' % (hour, minu, p, hsym)
                elif for_fun < 0.90:
                    last_name = '%s' % dizzy
                else:
                    last_name = '%s' % cake
